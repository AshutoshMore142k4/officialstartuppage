...
interface PixelCanvasProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Content layered above the canvas — pointer events over it still paint the trail */
  children?: React.ReactNode;
  /** Extra classes for the canvas element itself (e.g. opacity) — kept separate from `className` so dimming the effect never dims `children` */
...
export function PixelCanvas({
  ...
  return (
    <div
      ...
      {...props}
    >
      <canvas ref={canvasRef} aria-hidden="true" className={cn("absolute inset-0 block h-full w-full", canvasClassName)} />
      {children}
    </div>
  );
}
